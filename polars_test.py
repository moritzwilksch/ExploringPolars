...
if False:
    df = pl.scan_csv(root_path + f"{ticker}_tweets.csv")
    df.collect().to_parquet(root_path + f"{ticker}_tweets_clean.parquet")
else:
    df = pl.scan_parquet(root_path + f"{ticker}_tweets_clean.parquet").select(keep_cols)
...
def fix_dtypes(data):
    ...
    data = (
        data
        .with_column(pl.col("created_at").str.strptime(pl.Datetime, "%Y-%m-%d %H:%M:%S"))
        .drop(["date", "time"])
